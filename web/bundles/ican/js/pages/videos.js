var Videos = function () {
    ...
    var initTable = function () {
        MyApp.block('#video-table-editable');

        var table = $('#video-table-editable');
            ...
            },
            {
                ...
                template: function (row) {
                    ...
                    var output = '<div class="m-card-user m-card-user--sm">\
								<div class="m-card-user__pic m-card-video__pic">\
        ...
        oTable
            .on('m-datatable--on-ajax-done', function () {
                mApp.unblock('#video-table-editable');
            })
        ...
        $('#lista-video .m_form_search').on('keyup', function (e) {
    ...
    var resetForms = function () {
        $('#video-form input').each(function (e) {
            $element = $(this);
            $element.val('');
    ...
    var initAccionNuevo = function () {
        $(document).off('click', "#btn-nuevo-video");
        $(document).on('click', "#btn-nuevo-video", function (e) {
            btnClickNuevo();
        });

        function btnClickNuevo() {
            resetForms();
            var formTitle = "¿Deseas crear un nuevo video? Sigue los siguientes pasos:";
            $('#form-video-title').html(formTitle);
            $('#form-video').removeClass('m--hide');
            $('#lista-video').addClass('m--hide');
        };
    };
    //Salvar
    var initAccionSalvar = function () {
        ...
        function btnClickSalvarForm() {
            ...
            if ($('#video-form').valid() && $('#seo-form').valid() && imagen != "" && tags != "") {
                ...
                MyApp.block('#form-video');

                $.ajax({
                    type: "POST",
                    url: "video/salvarVideo",
                    dataType: "json",
                    data: {
                        'video_id': video_id,
                        'nombre': nombre,
    ...
    var initAccionCerrar = function () {
        $(document).off('click', ".cerrar-form-video");
        $(document).on('click', ".cerrar-form-video", function (e) {
            cerrarForms();
        });
    }
    //Cerrar forms
    ...
    var initAccionEditar = function () {
        $(document).off('click', "#video-table-editable a.edit");
        $(document).on('click', "#video-table-editable a.edit", function (e) {
            e.preventDefault();
            resetForms();
        ...
        function editRow(video_id) {

            MyApp.block('#video-form');

            $.ajax({
                type: "POST",
                url: "video/cargarDatos",
                dataType: "json",
                data: {
                    'video_id': video_id
                },
                success: function (response) {
                    mApp.unblock('#video-form');
                    if (response.success) {
                        //Datos video    

                        var formTitle = "¿Deseas actualizar la video \"" + response.video.nombre + "\" ? Sigue los siguientes pasos:";
                        $('#form-video-title').html(formTitle);

                        $('#nombre').val(response.video.nombre);
                        $('#titulo').val(response.video.titulo);
                        $('#descripcion').val(response.video.descripcion)

                        if (!response.video.estado) {
                        ...
                        mostrarImagen(response.video.imagen[0], response.video.imagen[1], response.video.imagen[2]);
    ...
    var initAccionEliminar = function () {
        $(document).off('click', "#video-table-editable a.delete");
        $(document).on('click', "#video-table-editable a.delete", function (e) {
            e.preventDefault();
        ...
        $(document).off('click', "#btn-eliminar-video");
        $(document).on('click', "#btn-eliminar-video", function (e) {
            btnClickEliminar();
        });
        ...
        function btnClickModalEliminar() {
            var video_id = rowDelete;

            MyApp.block('#video-table-editable');

            $.ajax({
                type: "POST",
                url: "video/eliminarVideo",
                dataType: "json",
                data: {
                    'video_id': video_id
                },
    ...
    var cerrarFormsConfirmated = function () {
        resetForms();
        $('#form-video').addClass('m--hide');
        $('#lista-video').removeClass('m--hide');
    };
            ...
            accept: function (file, done) {
                ...
                if (!file.type.match(/image.*/)) {
                    ...
                    if (maxFiles < realFiles) {
                        toastr.error('Solo se permite una imagen por video', "Error !!!");
                        //Eliminar
                        eliminarImagen(file);
    ...
    var initPortlets = function () {
        var portlet = new mPortlet('lista-video');
